...
9. 发送 50 点 MoveL 阶梯队列：X 总前进 `100 mm`、速度 `100 mm/s`，每批 25 点进行拼接。
0. 安全下电并退出。

菜单 `4`、`5`、`9` 会直接发送运动命令，不进行二次确认；执行前必须确保工作区安全。队列奇数点相对起点抬高 Z `5 mm`，偶数点回到起始 Z，每点沿 X 前进 `2 mm`。按本次调试范围，这些命令当前不等待运动完成，也不做可达性、运行队列、NaN/Inf 或超时重发检查。
